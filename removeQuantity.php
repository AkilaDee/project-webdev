...
<header class="header-section">
	...
	<div class="fpage">
		<div class="phead">
			<h2>Select Quantity to Remove</h2>
        </div>
        <form action="<?php echo ("{$_SERVER['PHP_SELF']}?item={$_GET['item']}"); ?>" method="post">	
            <div class="block2">
                <table>
                    ...
                    <!-- <tr>
                        <td><label for="date"><b>Please Select the Date:-  </b></label></td>
                        <td><input type="date" name="date" required></td>
                    </tr> -->
            
                </table>
            </br>
        <button type="submit" class="button1" name="submit">REMOVE</button>
        </form>
        <div style="color: red;">
        <?php 
            if(isset($_POST['submit'])) {
                ...
                if($conn-> query($sql) === TRUE) {
                    ...
                    $sql = "UPDATE inventory SET quantity = {$result['quantity']} + {$_POST['quantity']} WHERE 'item_name' = '{$_GET['item']}';";
                    if($conn->query($sql) === TRUE) {
                        echo "<script>alert('Inventory Removed');</script>";
                    }
                }
        ?>
        </div>
